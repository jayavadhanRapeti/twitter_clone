  ...
  if (jwtToken === undefined) {
    ...
    jwt.verify(jwtToken, "MY_SECRET_TOKEN", async (error, payload) => {
      if (error) {
        ...
        const loggedInUser = await db.get(userDetails);

        const { user_id } = loggedInUser;

        request.username = payload.username;
        request.user_id = loggedInUser.user_id;
        next();
      }
    });
  }
...
app.get("/user/following", authenticateToken, async (request, response) => {
  ...
  const getFollowingQuery = `
    ...
    `;
  const dbRequest = await db.all(getFollowingQuery);
  response.send(dbRequest);
});
...
app.get("/user/followers/", authenticateToken, async (request, response) => {
  ...
  const getFollowersQuery = `
    ...
    `;
  const dbRequest = await db.all(getFollowersQuery);
  response.send(dbRequest);
});
...
app.post("/user/tweets/", authenticateToken, async (request, response) => {
  ...
  const formattedDate = `${dateTime.getFullYear()}-${dateTime.getMonth()}-${dateTime.getDate()} ${dateTime.getHours()}:${dateTime.getMinutes()}:${dateTime.getSeconds()}`;
